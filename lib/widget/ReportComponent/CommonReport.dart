...
class HEAD2SLOTNEW extends StatelessWidget {
  HEAD2SLOTNEW({
    Key? key,
    this.ListFlex,
    this.widget01,
...
class HEAD2SLOTCUS extends StatelessWidget {
  HEAD2SLOTCUS({
    Key? key,
    this.ListFlex,
    this.hig,
  ...
  @override
  Widget build(BuildContext context) {
    List<int> _ListFlex = ListFlex ?? [1, 1];
    
    // Ensure we have at least 2 elements
    if (_ListFlex.length < 2) {
      _ListFlex = [1, 1];
    }
    return Padding(
      padding: EdgeInsets.only(right: padd ?? 8, left: padd ?? 8),
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                border: Border(
                  ...
                  left: BorderSide.none, // ไม่มีเส้นขอบด้านซ้าย
                ),
              ),
...
class BODY2SLOTCUS extends StatelessWidget {
  BODY2SLOTCUS({
    Key? key,
    this.ListFlex,
    this.hig,
...
class HEAD1SLOTTINY extends StatelessWidget {
  ...
  final int padding; // 👈 รับ padding เป็น int (pixels)
  final int height;  // 👈 รับ height เป็น int
  final List<int> ListFlex;

  @override
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(left: 8, right: 8), // 👈 ใช้ค่า padding ที่ส่งมา
      child: Row(
        children: [
          Expanded(
            flex: ListFlex[0], // 👈 ใช้ flex จาก ListFlex
            child: Container(
              height: height.toDouble(), // 👈 ใช้ค่า height ที่ส่งมา
              decoration: BoxDecoration(
                border: const Border(
                  top: BorderSide(color: Colors.black, width: 3),
...
class HEAD3SLOTWITHCHECKBOX extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    List<int> _ListFlex = ListFlex ?? [2, 3, 5];
    return Padding(
